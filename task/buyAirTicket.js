...
const searchTicket = async ({ time, formCityCode, toCityCode }) => {
  ...
  return new Promise((resolve) => {
    pageAir
      .goto(
        `http://www.ceair.com/booking/${formCityCode}-${toCityCode}-${time}_CNY.html`,
        {
      ...
      .then(async () => {
        let directAirlineList = await pageAir.evaluate(() => {
          ...
          return directAirline.map((item) => {
            ...
            let departureTime =
              from.querySelector("time") &&
              from.querySelector("time").textContent;
            let departureAirport = from.childNodes[1].textContent;
            let to = item.querySelectorAll(".airport")[1];
            let arrivalTime =
              to.querySelector("time") && to.querySelector("time").textContent;
...
const setBuyAirTicketPlan = (options) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (new Date(options.time) == "Invalid Date")
      resolve("请输入正确的时间格式：年-月-日");
    ...
    if (formCity.length > 1)
      resolve(
        `请输入详细出发城市名，比如${formCity.map((v) => v.title).join(",")}`
      );
    let toCity = searchCity(options.toCity);
    if (toCity.length == 0) resolve("找不到到达城市，请重新输入");
    if (toCity.length > 1)
      resolve(
        `请输入详细到达城市名，比如${toCity.map((v) => v.title).join(",")}`
      );
    let option = {
      time,
...
const sortDirectAirlineString = (directAirline) => {
  let data = `\ufeff`;
  directAirline.forEach(
    ({ departureTime, departureAirport, arrivalAirport, arrivalTime }) => {
